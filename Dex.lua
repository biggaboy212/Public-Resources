...
local function main()
    ...
    if not aClass then
        if a.Obj and a.Obj.ClassName then
            aClass = a.Obj.ClassName
            a.Class = aClass
        else
            return false -- Handle nil Obj or ClassName
        end
    end
    if not bClass then
        if b.Obj and b.Obj.ClassName then
    ...
    if not aOrder then
        if RMD.Classes[aClass] and tonumber(RMD.Classes[aClass].ExplorerOrder) then
            aOrder = tonumber(RMD.Classes[aClass].ExplorerOrder)
        else
            aOrder = 9999
        end
    ...
    if not bOrder then
        if RMD.Classes[bClass] and tonumber(RMD.Classes[bClass].ExplorerOrder) then
            bOrder = tonumber(RMD.Classes[bClass].ExplorerOrder)
        else
            bOrder = 9999
        end
...
Main = (function()
	...
	Main.FetchAPI = function()
		...
		if Main.Elevated then
			...
			rawAPI = rawAPI or game:HttpGet("http://setup.roblox.com/".. game:HttpGet("http://setup.roblox.com/versionQTStudio") .."-API-Dump.json")
			--rawAPI = Lib.ReadFile("dex/rbx_api.dat")
		else
