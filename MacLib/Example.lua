...
function MacLib:Window(Settings)
    ...
    local function UpdateOrientation(fetchProps)
        ...
        local z = camera.CFrame.ZVector*-0.001
        DrawQuad(
            camera:ScreenPointToRay(tl.x, tl.y).Origin + z,
            camera:ScreenPointToRay(tr.x, tr.y).Origin + z,
            camera:ScreenPointToRay(bl.x, bl.y).Origin + z,
            camera:ScreenPointToRay(br.x, br.y).Origin + z,
            parts
        )
        for _, pt in pairs(parts) do
            if pt then
